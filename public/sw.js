...
self.addEventListener("fetch", (event) => {
  ...
  // data), falling back to the intentionally generic offline page. We never
  // cache or replay an authenticated HTML response, so one account's page can
  // never become another account's offline document.
  if (request.mode === "navigate") {
    event.respondWith(
      fetch(request)
        // Never cache HTML: authenticated responses can contain private data.

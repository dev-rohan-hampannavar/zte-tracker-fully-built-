...
export default function WelcomePage() {
  ...
  // Older seeded orientation rows describe a 290-topic / 3034-hour source
  // document. The live tracker has since been reconciled; keep the welcome
  // tour from presenting those legacy figures as current product facts.
  const overview = useMemo(() => {
    const raw = orientation?.overview;
    if (!raw || /290-topic|3034|3,034|17\.5-month|2700|2,700/.test(raw)) {
      return "ZTE Tracker turns the Zero to Elite curriculum into a daily execution system. The Career Strategy page adds a personal 24-month operating plan: protect your income, ship proof, apply early, and make the final decision from evidence.";
    }
    return raw;
  }, [orientation]);
  ...
  const steps: StepDef[] = useMemo(() => {
    ...
    return list;
  }, [orientation, overview, whyThisWorks, masterPhaseTable, navigationNotes]);

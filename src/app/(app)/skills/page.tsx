...
import { useSkillEvidence, FRESHNESS_LABELS, FRESHNESS_VARIANT } from "@/lib/hooks/use-skills";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { pct, cn } from "@/lib/utils";
import { Layers, Gauge, Sparkles } from "lucide-react";
...
export default function SkillsPage() {
  ...
  const { data: skillEvidence, isLoading: skillsLoading } = useSkillEvidence(user?.id);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-1">
          Per-skill evidence and freshness, skill matrix by roadmap band, DSA coverage by tag, and phase readiness scores.
        </p>
      </div>
      </FadeUp>

      {/* Skill evidence — named skills (React, PostgreSQL, Docker, ...) with
          a knowledge % computed from real lesson completion
          (skill_evidence view, built on the existing technologies /
          topic_technologies tables), plus freshness/decay so stale skills
          surface for revision. This is additive to the band matrix below,
          not a replacement — band completion and named-skill evidence
          answer different questions. */}
      <Card className="glow-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Sparkles className="h-4 w-4" /> Skill evidence
          </CardTitle>
        </CardHeader>
        <StaggerContainer>
        <CardContent className="flex flex-col gap-3">
          {skillsLoading && <Skeleton className="h-32 w-full" />}
          {!skillsLoading && (skillEvidence ?? []).length === 0 && (
            <p className="text-sm text-muted">
              No skill evidence yet — complete roadmap topics that teach a tracked technology to build evidence here.
            </p>
          )}
          {(skillEvidence ?? []).map((s) => (
            <StaggerItem key={s.technology_id}>
            <div>
              <div className="flex items-center justify-between mb-1 gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <p className="text-sm font-medium truncate">{s.technology_name}</p>
                  {s.technology_category && (
                    <span className="text-[11px] text-muted shrink-0">{s.technology_category}</span>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Badge variant={FRESHNESS_VARIANT[s.freshness]} className="text-[10px]">
                    {FRESHNESS_LABELS[s.freshness]}
                  </Badge>
              ...
              <p className="text-[11px] text-muted mt-1">
                ...
                {s.last_reviewed_at &&
                  ` · last reviewed ${new Date(s.last_reviewed_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}`}
              </p>
            </div>
            </StaggerItem>
          ))}
          <p className="text-xs text-muted mt-2">
            Evidence here comes only from completed roadmap lessons for this technology. Project and interview
            evidence aren&apos;t factored in yet — DSA isn&apos;t either, since problem tags aren&apos;t linked to
            specific technologies, and guessing that link would show evidence that isn&apos;t really there.
          </p>
        </CardContent>
        </StaggerContainer>
      </Card>

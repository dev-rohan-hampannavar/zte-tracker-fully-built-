...
function TodayForm({ userId, todayLog, onSaved }: { userId: string; todayLog: DailyLog | undefined; onSaved: () => void }) {
  ...
  async function persist(values: { learned: string; mistakes: string; wins: string; tomorrowGoal: string }) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await persist({ learned, mistakes, wins, tomorrowGoal });
      onSaved();
  ...
  const debouncedAutosave = useDebouncedCallback(
    async (values: { learned: string; mistakes: string; wins: string; tomorrowGoal: string }) => {
  ...
  useEffect(() => {
    ...
    setAutosaveStatus("saving");
    debouncedAutosave({ learned, mistakes, wins, tomorrowGoal });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [learned, mistakes, wins, tomorrowGoal]);

...
export default function TechnologyDetailPage() {
  ...
  const mySkill = useMemo(() => (skillEvidence ?? []).find((s) => s.technology_id === params.id), [skillEvidence, params.id]);
  const { data: interviewQuestions } = useInterviewQuestions(undefined, params.id);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {mySkill && (
              <Badge variant={FRESHNESS_VARIANT[mySkill.freshness]}>
                Your evidence: {mySkill.knowledge_pct}% ({FRESHNESS_LABELS[mySkill.freshness]})
              </Badge>
            )}
          </div>
        </div>
      ...
      {(interviewQuestions ?? []).length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <ListChecks className="h-4 w-4" /> Interview questions
            </CardTitle>
            <p className="text-xs text-muted mt-1">Questions tagged with {technology.name} from the interview prep bank.</p>
          </CardHeader>
          <CardContent className="flex flex-col gap-2">
            {(interviewQuestions ?? []).map((q) => (
              <Link key={q.id} href="/interview-prep">
                <div className="flex items-center gap-3 rounded-card border border-border p-3.5 hover:border-accent/40 transition-standard">
                  <p className="text-sm flex-1 min-w-0">{q.question}</p>

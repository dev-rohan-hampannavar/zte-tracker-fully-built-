...
import { usePhasesWithProgress, useTechnologies, useAllTopicNotes, useLinkRegistry, useRoadmap, useExitLadder } from "@/lib/hooks/use-roadmap";
...
import { RecruiterEvidenceEditor } from "@/components/projects/recruiter-evidence-editor";
import { ProjectInterviewPrep } from "@/components/projects/project-interview-prep";
...
export default function ProjectsPage() {
  ...
  const exitByPhaseNumber = new Map((exitLadder ?? []).filter((e) => e.linked_phase).map((e) => [e.linked_phase, e]));
  ...
  async function handleSave(phaseId: string, patch: Record<string, unknown>, meta?: { prevStatus?: ProjectStatus; projectTitle?: string }) {
    if (!user) return;
    try {
      await upsertProjectProgress(user.id, phaseId, patch, meta);
      await mutate();
    } catch {
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="stage">
        <TabsList className="rounded-full bg-surface-2 p-1">
          ...
          <TabsTrigger
            ...
            className="rounded-full px-3.5 py-1.5 data-[state=active]:bg-accent data-[state=active]:text-accent-foreground data-[state=active]:shadow-none"
          >
            <FolderGit2 className="h-3.5 w-3.5 mr-1.5" /> Capstone builds ({phases.length})
          </TabsTrigger>
        </TabsList>
            ...
            <div key={phase.id} className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  aria-label={isPinned(settings?.pinned_items, "project", phase.id) ? "Unpin from Workspace" : "Pin to Workspace"}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {phases.map((phase) => {
          ...
          const linkedExit = exitByPhaseNumber.get(phase.phase_number);
            ...
            <Card key={phase.id}>
              <CardHeader>
                ...
                {linkedExit && (
                  <p className="text-[11px] text-accent mt-1">
                    Required for {linkedExit.exit_code}
                    {linkedExit.name ? ` — ${linkedExit.name}` : ""}
                    {status !== "completed" && " · not yet complete"}
                  </p>
                )}
                <CardDescription>Capstone build for this phase</CardDescription>
                {(p?.started_at || p?.completed_at) && (
                  <p className="text-[11px] text-muted -mt-1">
                    {p?.started_at && <>Started {formatDueDate(p.started_at)}</>}
                    {p?.started_at && p?.completed_at && " · "}
                    {p?.completed_at && <>Completed {formatDueDate(p.completed_at)}</>}
                  </p>
                )}
                {phaseTech.length > 0 && (
                ...
                <ProjectSkillConfirm
                  ...
                  phaseId={phase.id}
                  suggested={phaseTech}
                  confirmed={(projectSkills ?? []).filter((ps) => ps.phase_id === phase.id)}
              ...
              <CardContent className="flex flex-col gap-3">
                <Select
                  value={status}
                  onValueChange={(v) =>
                    handleSave(phase.id, { status: v as ProjectStatus }, { prevStatus: status, projectTitle: phase.title })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                ...
                <RecruiterEvidenceEditor project={p} onSave={(patch) => handleSave(phase.id, patch)} />
                ...
                <ProjectInterviewPrep
                  ...
                  project={{
                    ...
                    architecture_notes: p?.architecture_notes ?? null,
                    contribution_notes: p?.contribution_notes ?? null,
                    challenges_notes: p?.challenges_notes ?? null,
                    tradeoffs_notes: p?.tradeoffs_notes ?? null,
                    metrics_notes: p?.metrics_notes ?? null,
                    testing_notes: p?.testing_notes ?? null,
                    technologies: (technologies ?? []).filter((t) =>
                      (projectSkills ?? []).some((ps) => ps.phase_id === phase.id && ps.technology_id === t.id)
                    ),
                  }}
                />
                ...
                <div className="flex gap-3 text-xs">
                  {p?.github_url && (
                    <a href={p.github_url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline flex items-center gap-1">
                      <GitBranch className="h-3 w-3" /> Repo
                    </a>
                  )}
                  {p?.deployment_url && (
                    <a href={p.deployment_url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline flex items-center gap-1">
                      <ExternalLink className="h-3 w-3" /> Live
                    </a>
                  )}
                  {p?.demo_url && (
                    <a href={p.demo_url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline flex items-center gap-1">

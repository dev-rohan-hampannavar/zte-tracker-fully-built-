...
import { ReadinessBadge } from "@/components/roadmap/readiness-badge";
...
function PhaseCardGrid({
  ...
  const readinessByPhase = useMemo(() => computePhaseReadiness(phases), [phases]);
  const scheduleByPhase = useMemo(() => computePhaseScheduleStatus(phases, actualHours), [phases, actualHours]);
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <ReadinessBadge readiness={readinessByPhase.get(openPhase.id)} />
            {(() => {
              const sched = scheduleByPhase.get(openPhase.id);
              if (!sched) return null;
              const label = sched.status === "ahead" ? "Ahead of schedule" : sched.status === "behind" ? "Behind schedule" : "On pace";
              const color =
                sched.status === "ahead"
              ...
              return (
                <Badge variant="outline" className={cn("text-xs", color)} title={`${Math.abs(Math.round(sched.hoursDelta))}h ${sched.hoursDelta >= 0 ? "ahead of" : "behind"} plan pace`}>
                  {label}
                </Badge>
              );
            })()}
          ...
          >
            <CardContent noHeader className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <ReadinessBadge readiness={readinessByPhase.get(phase.id)} />
...
function RoadmapListView() {
  ...
  const actualHours = useMemo(
    () => (dailyLogs ?? []).reduce((sum, l) => sum + Number(l.hours), 0),
    [dailyLogs]
  );
  ...
  async function handleToggle(topicId: string, completed: boolean) {
    ...
    try {
      const topic = phases
        ...
        .flatMap((s) => s.topics)
        .find((t) => t.id === topicId);
      await toggleTopicComplete(user.id, topicId, completed, topic?.title);
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex items-center gap-0.5 rounded-full border border-border bg-surface-2 p-1 shrink-0 flex-wrap">
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium transition-standard",
              viewMode === "timeline" ? "bg-accent text-accent-foreground shadow-sm" : "text-muted hover:text-foreground"
            )}
            title="24-Month Timeline"
          >
                ...
                <div className="flex-1 flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Progress value={pct(completedCount, totalCount)} className="h-1.5 w-32" glow={completedCount === totalCount && totalCount > 0} />
                      <span className="text-xs text-muted font-mono-tabular shrink-0">
                        {completedCount}/{totalCount}
                      </span>
                      <Link
...
// Thin tab router — the full roadmap body above (RoadmapListView, ~950
// lines of accordion/search/filter logic) is unchanged; this only adds a
// second tab for the Learning Path / dependency graph view that used to
// be its own top-level page at /dependency-graph before the IA
// consolidation merged it in here. Tab state lives in the URL (?tab=) so
// the redirect from the old route (see next.config.ts) lands on the right
// tab, and so the tab choice survives a refresh/back-button.
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="text-sm text-muted">
        The sequential order phases unlock in, grouped by band. Diamond nodes are exit points — real
        off-ramps to apply, not just checkpoints.
      </p>

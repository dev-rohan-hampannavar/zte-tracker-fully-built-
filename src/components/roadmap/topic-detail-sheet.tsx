...
export function TopicDetailSheet({
  ...
  useEffect(() => {
    ...
    setEvidenceUrlInput(topic.progress?.evidence_url ?? "");
  }, [topic, user]);
  ...
  async function handleSaveEvidenceUrl() {
    if (!user || !topic) return;
    await updateTopicProgress(user.id, topic.id, { evidence_url: evidenceUrlInput.trim() || null });
    onUpdated();
    toast.success("Evidence link saved");
  }
    ...
    <Dialog open={!!topic} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted mb-1">Commit, PR, or repo link proving this topic was actually built, not just read.</p>
          <div className="flex gap-2 mt-1">
            <Input
              ...
              className="flex-1"
            />
            <Button size="sm" variant="secondary" onClick={handleSaveEvidenceUrl}>
              Save
            </Button>
          </div>
          {topic.progress?.evidence_url && (
            <a
              href={topic.progress.evidence_url}
              target="_blank"

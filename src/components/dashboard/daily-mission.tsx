  ...
  async function handleLogSession(e: React.FormEvent) {
    ...
    try {
      ...
      await logStudySessionEntry(userId, {
        ...
        isTutorial: activity === "learn" || activity === "project" ? isTutorial : null,
      });
      await onMutateLogs();
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleLogSession} className="flex flex-col gap-4">
            ...
            {(activity === "learn" || activity === "project") && (
              <div className="flex items-center justify-between rounded-lg border border-border/40 px-3 py-2">
                <div>
                  <Label className="cursor-pointer" htmlFor="is-tutorial-toggle">
                    Following a tutorial/course?
                  </Label>
                  <p className="text-[11px] text-muted">Helps track tutorial vs. independent building over time.</p>
                </div>
                <Switch id="is-tutorial-toggle" checked={isTutorial === true} onCheckedChange={(v) => setIsTutorial(v)} />
              </div>
            )}
            <div>

...
const NAV_SHORTCUTS: { keys: string; label: string; href?: string }[] = [
  ...
  // The three below are genuinely page-scoped actions (start focus / log
  // time only exist on the dashboard's FocusTimer; new task only exists
  // on /daily-plan's manual-add form) rather than app-wide capabilities —
  // implemented as navigation to where that action lives, same as every
  // other entry here, rather than a blind cross-page dispatch that would
  // need new query-param plumbing on each target page just for this.
  { keys: "G then T", label: "Go to Daily Plan (add a task)", href: "/daily-plan" },
  { keys: "G then F", label: "Go to Dashboard (start focus)", href: "/dashboard" },
  { keys: "G then L", label: "Go to Dashboard (log time)", href: "/dashboard" },
